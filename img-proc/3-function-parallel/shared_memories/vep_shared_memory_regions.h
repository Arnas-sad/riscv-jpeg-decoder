...
#define TOK_COL_SIZE   3200u   /* >= sizeof(tok_col_t):  sh2 + ColorBuffer        */
typedef volatile struct {
  uint32_t initialized;
  fifo_t   f_coef;  uint8_t b_coef[PIPE_DEPTH * TOK_COEF_SIZE];  /* VLD  -> IQZZ */
  fifo_t   f_freq;  uint8_t b_freq[PIPE_DEPTH * TOK_FREQ_SIZE];  /* IQZZ -> IDCT */
  fifo_t   f_pix;   uint8_t b_pix [PIPE_DEPTH * TOK_PIX_SIZE];   /* IDCT -> CC   */
  fifo_t   f_col;   uint8_t b_col [PIPE_DEPTH * TOK_COL_SIZE];   /* CC   -> rast */
...
/* 2c raster sink (tile 2 partition 2) DMA staging: tile_buf is rasterised into
   and DMAed to the DRAM scratch area; dma_scratch carries the geometry header
   (and the TOC for the gather's done-signal); input_block is the gather pass's
   scratch-block read cache. Each field is a 1 KiB-aligned DMA endpoint, filling
   the 3 KiB region exactly. */
typedef volatile struct {
  uint8_t dma_scratch[1024];
  uint8_t tile_buf[1024];
  uint8_t input_block[1024];
