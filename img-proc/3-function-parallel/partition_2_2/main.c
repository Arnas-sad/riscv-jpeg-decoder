/*
 * JPEG FUNCTION-PARALLEL decoder (2c) - RASTER SINK (tile 2, partition 2).
 *
 * Pipeline tail: f_col in -> rasterise tile-local -> DMA to the DRAM scratch
 * area (append-counter packing; MCUs arrive in raster order). On the `last`
 * sentinel it publishes the geometry header for the ARM scatter. Only stage
 * with no output FIFO; its "output" is the DRAM scratch.
 *
 * Benchmarking: wait_in (starved for tokens) vs work (copy + raster + DMA wait)
 * on the partition timer; one BENCH block at the end. The frame latency is
 * derived offline as raster.gend - vld.gstart. 64-bit totals in kibicycles.
 */
#include <stdint.h>
#include <string.h>
#include <timers.h>
...
int main(void) {
    xil_printf("%d %d: 2c raster sink starting\n", TILE_ID, PARTITION_ID);

    while (!fifo_initialized(&MEM->f_col)) asm("wfi");
    ...
    for (;;) {
        ...
        tok_col_t volatile *in = (tok_col_t volatile *)fifo_claim_token(&MEM->f_col);
        ...
        if (!last) {
            sh2 = in->sh2;
            /* S-02: read only the bytes CC actually wrote (raster + the
               checksum only touch this range). Mono = one 8x8 block. */
            size_t cb_bytes = (sh2.n_comp == 1)
                            ? (size_t)(sh2.MCU_sx * sh2.MCU_sy)
                            : (size_t)(3 * sh2.MCU_sx * sh2.MCU_sy);
            memcpy(cbloc.data, (const void *)in->cb.data, cb_bytes);
        }
        fifo_release_space(&MEM->f_col);   /* always drain so upstream never stalls */
        if (last) break;

        if (first) {
            tile_emit_init(sh2.x_size, sh2.y_size, sh2.MCU_sx, sh2.MCU_sy, sh2.n_comp);
            /* Checksum the first MCU (post-CC) as an end-to-end correctness
               signal through all 4 FIFOs - must be 74240 for cat.jpg. */
        ...
        /* On out-of-scope (fb too big / scratch full) we keep draining tokens
           but stop emitting, so the pipeline still terminates cleanly. */
        if (!tile_emit_failed()) { tile_emit_mcu(&sh2, &cbloc); n++; }
     ...
     * signal done by zeroing the TOC word `dram.sh poll` watches. Single sink
     * partition -> no barrier. Done before g_end so RT covers composition; the
     * done-signal is the WCRT endpoint (spec p29: RISC-V signals output ready). */
    if (!tile_emit_failed()) {
    ...
    xil_printf("%d %d: raster done - %u MCUs in %u ms\n",
               TILE_ID, PARTITION_ID, n, (uint32_t)(rt / (FREQUENCY / 1000)));

    if (fail == 0) {
        xil_printf("BENCH impl=function-parallel stage=raster tile=%d partition=%d n_mcus=%u jpeg_size=0\n",
