 ...
 *
 * 2c sink build: compiled UNCONDITIONALLY (no JPEG_DRAM_INPUT guard), like the
 * sink's own tile_emit.c - the raster sink always runs on the board and drives
 * the DMA engine.
 */
#include <stdint.h>
#include <dma.h>
...
/* The gather is DMA-bound, so spin on the queue rather than wfi: a wfi sleeps
   until the next TDM slot (~67k cyc/transfer); the transfer itself is ~300 cyc,
              ...
              volatile uint8_t *cache_buf, volatile uint8_t *out_buf)
{
  ...
  /* Block 0 is the header, never a tile block; cached=0 forces a load on the
     first MCU (tile blocks are 1-based). */
  uint32_t cached = 0;
  ...
  for (uint32_t blk = 0; blk < n_blocks; blk++) {
    ...
    uint32_t p0   = blk * FB_WORDS_PER_BLOCK;
    uint32_t y    = p0 / xs;
    uint32_t x    = p0 - y * xs;
    uint32_t mrow = y / sy, i = y - mrow * sy;
    uint32_t mcol = x / sx, j = x - mcol * sx;
...
{
  /* Read-modify-write the TOC block so only word 0 (the file count dram.sh poll
     watches) is cleared; words 1..7 of the TOC are preserved. */
