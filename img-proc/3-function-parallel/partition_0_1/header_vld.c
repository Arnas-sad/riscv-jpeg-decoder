...
void init_header_vld (VldHeader *header, const unsigned int *data, unsigned int *fb)
{
...
#ifdef VERBOSE
  fprintf (stderr, "%d:\tINFO:\tFound the SOI marker!\n", FTELL (&(header->fp)));
#endif
  ...
  do {
    ...
    switch (mark)
    {
      case SOF_MK:
#ifdef VERBOSE
	fprintf (stderr, "%d:\tINFO:\tFound the SOF marker!\n", FTELL (&(header->fp)));
#endif
	header->in_frame = 1;
	get_size (&(header->fp));                    /* header size, don't care */

	/* load basic image parameters */
	(void) FGETC (&(header->fp));                /* precision, 8bit, don't care */
	...
	break;
      ...
      case DHT_MK:
#ifdef VERBOSE
	fprintf (stderr, "%d:\tINFO:\tDefining Huffman Tables\n", FTELL (&(header->fp)));
#endif
	...
	break;
      ...
      case DQT_MK:
#ifdef VERBOSE
	fprintf (stderr, "%d:\tINFO:\tDefining Quantization Tables\n", FTELL (&(header->fp)));
#endif
	...
	break;
      case COM_MK:
#ifdef VERBOSE
	fprintf (stderr, "%d:\tINFO:\tSkipping comments\n", FTELL (&(header->fp)));
#endif
...
#ifdef VERBOSE
	fprintf (stderr, "%d:\tINFO:\tDefining Restart Interval %d\n", FTELL (&(header->fp)), header->restart_interval);
#endif
	break;

      case SOS_MK:
#ifdef VERBOSE
	fprintf (stderr, "%d:\tINFO:\tFound the SOS marker!\n", FTELL (&(header->fp)));
#endif
	get_size (&(header->fp));
	aux = FGETC (&(header->fp));
	...
	break;
      ...
      case EOI_MK:
#ifdef VERBOSE
	fprintf (stderr, "%d:\tINFO:\tFound the EOI marker before any MCU is obtained!\n", FTELL (&(header->fp)));
#endif
	aborted_stream (&(header->fp));
	break;
      ...
      case EOF:
#ifdef VERBOSE
	fprintf (stderr, "%d:\tERROR:\tRan out of input data!\n", FTELL (&(header->fp)));
#endif
	aborted_stream (&(header->fp));
	break;
...
#ifdef VERBOSE
	  fprintf (stderr, "%d:\tINFO:\tSkipping application data\n", FTELL (&(header->fp)));
#endif
	  skip_segment (&(header->fp));
	  break;
	}
	...
	fprintf (stderr, "%d:\tWARNING:\tLost Sync outside scan, %d!\n", FTELL (&(header->fp)), mark);
	aborted_stream (&(header->fp));
	break;
    }                                                       // end switch
  } while (1);
...
int header_vld (VldHeader * header, VldHeader * updated_header, FValue mcu_after_vld[10], SubHeader1 * subheader1, SubHeader2 * subheader2)
{
  unsigned int aux, mark;
  int          end = 0;
  ...
  if (header->mcu_rst > 0) {
    ...
    if (header->mcu_rst % header->restart_interval == 0) {
      aux = get_next_MK (&(header->fp));
      if (!RST_MK (aux)) {
	fprintf (stderr, "%d:\tERROR:\tLost Sync after interval!\n", FTELL (&(header->fp)));
	aborted_stream (&(header->fp));
      }
#ifdef VERBOSE
      fprintf (stderr, "%d:\tINFO:\tFound Restart Marker\n", FTELL (&(header->fp)));
#endif
      reset_prediction (header->comp);
      clear_bits ();
    }
  ...
  else if (header->leftover > 0) {          // still more MCUs to process
    load_MCU (header, mcu_after_vld);
    header->leftover--;
    copy_subheaders (header, subheader1, subheader2);
    return end;
	...
	case EOI_MK:
#ifdef VERBOSE
	  fprintf (stderr, "%d:\tINFO:\tFound the EOI marker!\n", FTELL (&(header->fp)));
#endif
...
int load_MCU (VldHeader *header, FValue mcu_after_vld[10])
{
...
unsigned int get_num_pixels (VldHeader *header)
{
