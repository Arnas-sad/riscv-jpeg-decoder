 ...
 * Compiled into the decode only on the board build (-DJPEG_DRAM_INPUT), like
 * tile_emit.c, because it depends on the DMA engine and DRAM addresses.
 */
...
#else
typedef int fb_gather_translation_unit_not_empty;
