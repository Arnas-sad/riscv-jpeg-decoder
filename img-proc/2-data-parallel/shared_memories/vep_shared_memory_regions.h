...
/* Data-parallel (2b) done-barrier. Each cooperating tile sets done[TILE_ID]=1
   after it has flushed all its scratch tiles (a plain volatile store over the
   memory ring - no DMA); tile 0 spins until all N_TILES flags are set, then it
   alone runs the gather. memshared0 is zeroed at load, so done[] starts clear. */
